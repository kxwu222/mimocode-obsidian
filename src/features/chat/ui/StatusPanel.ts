...
export class StatusPanel {
  ...
  private createPanel(): void {
    ...
    this.bashOutputContainerEl = this.panelEl.createDiv({
      cls: 'claudian-status-panel-bash claudian-hidden',
    });
    ...
    this.bashContentEl = this.bashOutputContainerEl.createDiv({
      cls: 'claudian-status-panel-bash-content',
    });

    this.todoContainerEl = this.panelEl.createDiv({
      cls: 'claudian-status-panel-todos claudian-hidden',
    });
    ...
    if (!this.isTodoExpanded) {
      if (completedCount === totalCount && totalCount > 0) {
        const status = this.todoHeaderEl.createSpan({
          cls: 'claudian-status-panel-status status-completed',
        });
  ...
  private renderBashOutputs(options: { scroll?: boolean } = {}): void {
    ...
    this.bashHeaderEl.createSpan({
      cls: 'claudian-tool-label',
      text: this.isBashExpanded
        ? t('chat.bangBash.commandPanel')
        : latest
          ? this.truncateDescription(latest.command, 60)
          : t('chat.bangBash.commandPanel'),
    });
    ...
    for (const info of this.currentBashOutputs.values()) {
      this.renderBashEntry(this.bashContentEl, info);
    }
  ...
  private renderBashEntry(parent: HTMLElement, info: PanelBashOutput): HTMLElement {
