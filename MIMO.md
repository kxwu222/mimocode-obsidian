...
- Mimo adaptor: `src/providers/mimo/` owns the HTTP runtime, settings (billing mode, API key, cluster, model, web search), capabilities, local transcript persistence, settings reconciler, chat UI config, settings tab, and auxiliary services (title gen, inline edit via `MimoAuxQueryRunner`). Chat turns can call built-in vault tools (`Read`, `LS`, `Glob`, `Grep`, `Write`, `Edit`, `Delete`) and, when enabled, MiMo's built-in `web_search` plugin. Delete uses Obsidian trash. Instruction-mode and MCP tool use stay disabled until they have a real persistence and settings path.
